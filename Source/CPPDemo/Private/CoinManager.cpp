...
void ACoinManager::SpawnCoins()
{
    ...
    {
        //ATargetPoint* SpawnPoint = Cast<ATargetPoint>(SpawnPoints[i]);

        //if (SpawnPoint)
        //{
            // Spawn a coin at each TargetPoint
...
void ACoinManager::CoinCounter()
{
	...
	if (GEngine)
	{
		GEngine->AddOnScreenDebugMessage(-1, 2.0f, FColor::Yellow, FString::Printf(TEXT("Goal: %i / %i"), currentCoinsCollected, totalCount));
	}

	
    if (currentCoinsCollected >= totalCount)
	{
		if (GEngine)
		{
